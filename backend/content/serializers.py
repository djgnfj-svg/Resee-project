...
class ContentSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if isinstance(category, int):
            try:
                category = Category.objects.get(
                    id=category, user=self.context["request"].user
                )
                validated_data["category"] = category
            except Category.DoesNotExist:
                raise serializers.ValidationError(
                    {"category": "Category not found or doesn't belong to user"}
                )
        ...
        if content.review_mode == "multiple_choice":
            ...
            mc_options = generate_multiple_choice_options(
            ...
            if mc_options:
                ...
                logger.warning(
                    f"Failed to generate MC options for content {content.id}"
                )
    ...
    def update(self, instance, validated_data):
        ...
        content_changed = (
            ...
            and validated_data["content"] != instance.content
        )
        ...
        if content.review_mode == "multiple_choice" and (
            title_changed or content_changed
        ):
            from ai_services import generate_multiple_choice_options
            ...
            if mc_options:
                ...
                logger.warning(
                    f"Failed to regenerate MC options for content {content.id}"
                )
