...
class CategoryViewSetTest(TestCase):
    ...
    def test_create_category(self):
        ...
        response = self.client.post(
            "/api/categories/", {"name": "Python", "description": "Python programming"}
        )
    ...
    def test_create_category_limit_exceeded(self):
        ...
        response = self.client.post("/api/categories/", {"name": "TooMany"})
    ...
    def test_update_category(self):
        ...
        response = self.client.put(
            ...
            {"name": "Python 3", "description": "Updated description"},
        )
        self.assertIn(
...
class ContentViewSetTest(TestCase):
    ...
    def test_create_content_limit_exceeded(self):
        ...
        for i in range(20):
            Content.objects.create(
                title=f"Content{i}", content=f"Content {i}", author=self.user
            )
        ...
        self.assertEqual(
            content_count, 20, f"Expected 20 contents, got {content_count}"
        )
        self.assertEqual(
            perm.get_content_limit(),
            20,
            f"Expected limit 20, got {perm.get_content_limit()}",
        )
        self.assertFalse(
            perm.can_create_content(), "Should not be able to create more content"
        )

        # Try to create one more (21st)
        response = self.client.post(
            "/api/contents/", {"title": "TooMany", "content": "Too many contents"}
        )
        self.assertEqual(response.status_code, status.HTTP_402_PAYMENT_REQUIRED)
        self.assertIn("error", response.data)
    ...
    def test_retrieve_content(self):
        ...
        content = Content.objects.create(
            title="Test", content="Test content", author=self.user
        )
    ...
    def test_search_content(self):
        ...
        Content.objects.create(
            title="Python Basics", content="Python programming", author=self.user
        )
        Content.objects.create(
            title="Django Tutorial", content="Web framework", author=self.user
        )
    ...
    def test_validate_content_action(self, mock_validate):
        ...
        response = self.client.post(
            "/api/contents/validate/", {"title": "Python Basics", "content": "x" * 250}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_validate_content_too_short(self):
        ...
        response = self.client.post(
            "/api/contents/validate/", {"title": "Python", "content": "Short"}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("error", response.data)
    ...
    def test_validate_and_save_action(self, mock_validate):
        ...
        content = Content.objects.create(
            title="Test", content="x" * 250, author=self.user
        )
    ...
    def test_validate_and_save_too_short(self):
        ...
        content = Content.objects.create(
            title="Test", content="Short", author=self.user
        )
