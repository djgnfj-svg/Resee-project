...
class AILogger:
    ...
    def log_question_generation(
        ...
        self.logger.info(
            f"AI Question Generation: user={user_id}, content={content_id}, "
    ...
    def log_answer_evaluation(
        ...
        self.logger.info(
            f"AI Answer Evaluation: user={user_id}, question={question_id}, "
            f"result={evaluation_result}, success={success}"
        )

    def log_ai_service_error(
        self, service: str, error: str, user_id: Optional[int] = None
    ):
        """Log AI service errors"""
        self.logger.error(
            f"AI Service Error: service={service}, error={error}, user={user_id}"
        )
...
def log_api_call(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.info(
                f"API Call: {func.__name__} completed successfully in {duration:.2f}ms"
            )
            return result
            ...
            logger.error(
                f"API Call: {func.__name__} failed after {duration:.2f}ms - {str(e)}"
            )
            raise
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                duration = (end_time - start_time) * 1000
                logger.error(
                    f"Performance: {operation_name} failed after {duration:.2f}ms - {str(e)}"
                )
                raise
...
def log_user_action(
    ...
    """Log user actions"""
    log_data = {"action": action, "user_id": user_id, "timestamp": time.time()}
    if metadata:
...
class BusinessMetricsLogger:
    ...
    def log_subscription_event(
        self, user_id: int, event_type: str, tier: str, metadata: Optional[Dict] = None
    ):
        """Log subscription-related events"""
        self.logger.info(
            f"Subscription Event: user={user_id}, type={event_type}, "
    ...
    def log_content_event(
        ...
        self.logger.info(
            f"Content Event: user={user_id}, content={content_id}, "
