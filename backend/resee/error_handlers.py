...
class APIErrorHandler:
    ...
    def validation_error(
        ...
        """
        return Response(
            APIErrorHandler._format_error(ErrorCode.VALIDATION_ERROR, message, errors),
            status=status.HTTP_400_BAD_REQUEST,
        )

    @staticmethod
    def permission_denied(
        message: str = "Permission denied", details: Optional[Dict] = None
    ):
        return Response(
            APIErrorHandler._format_error(
                ErrorCode.PERMISSION_DENIED, message, details
            ),
    ...
    @staticmethod
    def unauthorized(
        message: str = "Authentication required", details: Optional[Dict] = None
    ):
        return Response(
    ...
    def server_error(
        ...
        return Response(
            APIErrorHandler._format_error(
                ErrorCode.INTERNAL_SERVER_ERROR, message, details
            ),
    ...
    def custom_error(
        ...
        return Response(
            APIErrorHandler._format_error(code, message, details), status=status_code
        )

    @staticmethod
    def rate_limit_exceeded(
        message: str = "Rate limit exceeded", retry_after: Optional[int] = None
    ):
        details = {"retry_after": retry_after} if retry_after else None
        return Response(
            APIErrorHandler._format_error(
                ErrorCode.RATE_LIMIT_EXCEEDED, message, details
            ),
        ...
        try:
            ...
            return APIErrorHandler.server_error(
                ...
                details=(
                    ...
                    if not getattr(args[0].request, "user", None)
                    else None
                ),
            )
