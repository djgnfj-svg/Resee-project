            ...
            with connection.cursor() as cursor:
                try:
                    ...
                    if estimated_count > 10000:
                        logger.info(
                            f"Using estimated count for {table_name}: {estimated_count}"
                        )
                        # Use custom paginator that doesn't count all rows
                        return self._paginate_with_estimation(
                            queryset, request, estimated_count
                        )

                except Exception as e:
                    logger.warning(
                        f"Failed to get estimated count for {table_name}: {e}"
                    )
    ...
    def get_paginated_response(self, data):
        ...
        if data:
            ...
            response.data["review_meta"] = {
                "completion_rate": (
                    round(completed_reviews / total_reviews * 100, 1)
                    if total_reviews > 0
                    else 0
                ),
