...
def performance_metrics(request):
    ...
    **Requires:** Admin authentication
    ...
    return Response(
        {"timestamp": timezone.now().isoformat(), "performance": perf_metrics},
    ...
    **Requires:** Admin authentication
    ...
    metrics = {
        ...
        "content": {
            ...
            "created_this_month": Content.objects.filter(
                created_at__gte=month_ago
            ).count(),
        },
        "reviews": {
            "total_scheduled": ReviewSchedule.objects.filter(
                completed_at__isnull=True
            ).count(),
            "due_today": ReviewSchedule.objects.filter(
                next_review_date__date=today, completed_at__isnull=True
            ).count(),
...
def _get_application_metrics() -> Dict[str, Any]:
    ...
    return {
        ...
        "environment": os.environ.get("DJANGO_SETTINGS_MODULE", "unknown").split(".")[
            -1
        ],
