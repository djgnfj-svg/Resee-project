...
class GDPRDataExportSerializer(serializers.Serializer):
    ...
    def to_representation(self, instance):
        ...
        # 결제 기록 - 구독 정보에서 가져옴
        if hasattr(user, 'subscription') and user.subscription and user.subscription.amount_paid:
            user_data['payment_history'] = [
                {
                    'amount': str(user.subscription.amount_paid),
                    'plan_type': user.subscription.tier,
                    'status': 'active' if user.subscription.is_active else 'inactive',
                    'created_at': user.subscription.created_at.isoformat(),
                    'updated_at': user.subscription.updated_at.isoformat(),
                }
            ]
