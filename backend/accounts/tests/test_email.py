...
class EmailVerificationViewTest(TestCase):
    ...
    def test_verify_email_success(self):
        ...
        response = self.client.post(
            "/api/accounts/verify-email/", {"token": token, "email": "test@example.com"}
        )
    ...
    def test_verify_email_invalid_token(self):
        ...
        response = self.client.post(
            ...
            {"token": "invalid_token", "email": "test@example.com"},
        )
    ...
    def test_verify_email_missing_fields(self):
        ...
        response = self.client.post(
            "/api/accounts/verify-email/", {"token": "some_token"}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_verify_email_nonexistent_user(self):
        ...
        response = self.client.post(
            "/api/accounts/verify-email/",
            {"token": "some_token", "email": "nonexistent@example.com"},
        )
        self.assertIn(
...
class ResendVerificationViewTest(TestCase):
    ...
    def test_resend_verification_success(self):
        ...
        response = self.client.post(
            "/api/accounts/resend-verification/", {"email": "test@example.com"}
        )
    ...
    def test_resend_verification_nonexistent_user(self):
        ...
        response = self.client.post(
            "/api/accounts/resend-verification/", {"email": "nonexistent@example.com"}
        )
        self.assertIn(
            response.status_code,
            [status.HTTP_400_BAD_REQUEST, status.HTTP_404_NOT_FOUND],
