...
class ProfileViewTest(TestCase):
    ...
    def test_update_profile(self):
        ...
        response = self.client.put(
            ...
            {"email": "test@example.com", "username": "newusername"},
            format="json",
        )
        self.assertIn(
...
class WeeklyGoalUpdateViewTest(TestCase):
    ...
    def test_update_weekly_goal(self):
        ...
        response = self.client.put(
            "/api/accounts/weekly-goal/", {"weekly_goal": 10}, format="json"
        )
        self.assertIn(
            response.status_code, [status.HTTP_200_OK, status.HTTP_400_BAD_REQUEST]
        )
    ...
    def test_update_weekly_goal_invalid(self):
        ...
        response = self.client.put(
            "/api/accounts/weekly-goal/", {"weekly_goal": -1}, format="json"
        )
...
class NotificationPreferenceViewTest(TestCase):
    ...
    def test_create_notification_preferences(self):
        ...
        response = self.client.post(
            ...
            {"email_notifications_enabled": True, "daily_reminder_enabled": True},
            format="json",
        )
    ...
    def test_update_notification_preferences(self):
        ...
        response = self.client.put(
            "/api/accounts/notification-preferences/",
            {"email_notifications_enabled": False, "daily_reminder_enabled": False},
            format="json",
        )
        self.assertIn(
            response.status_code, [status.HTTP_200_OK, status.HTTP_400_BAD_REQUEST]
