...
class GoogleAuthServiceTest(TestCase):
    ...
    @override_settings(
        SOCIALACCOUNT_PROVIDERS={"google": {"APP": {"client_id": "test-client-id"}}}
    )
    ...
    @override_settings(SOCIALACCOUNT_PROVIDERS={"google": {"APP": {"client_id": ""}}})
    ...
    def test_get_or_create_user_existing(self):
        ...
        user = User.objects.create_user(
            email="existing@example.com", password="pass123", is_email_verified=False
        )
    ...
    def test_get_or_create_user_name_parsing(self):
        ...
        google_info = {"email": "test@example.com", "name": "Full Name"}
...
class GoogleOAuthViewTest(TestCase):
    ...
    def test_google_oauth_success(self, mock_get_user, mock_verify):
        ...
        user = User.objects.create_user(
            email="test@example.com", is_email_verified=True
        )
        ...
        response = self.client.post(
            "/api/accounts/google-oauth/", {"token": "valid-google-token"}
        )
    ...
    def test_google_oauth_invalid_token(self, mock_verify):
        ...
        response = self.client.post(
            "/api/accounts/google-oauth/", {"token": "invalid-token"}
        )
