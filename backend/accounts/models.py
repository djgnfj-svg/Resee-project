...
class User(AbstractUser):
    ...
    username = models.CharField(
        max_length=150, blank=True, null=True, help_text="Optional username field"
    )
    ...
    email_verification_token = models.CharField(
        max_length=255, blank=True, null=True, help_text="Token for email verification"
    )
    ...
    # 학습 목표 설정
    weekly_goal = models.IntegerField(default=7, help_text="Weekly review goal count")

    # 약관 동의 관련 필드
    terms_agreed_at = models.DateTimeField(
        null=True, blank=True, help_text="Terms of service agreement timestamp"
    )
    privacy_agreed_at = models.DateTimeField(
        null=True, blank=True, help_text="Privacy policy agreement timestamp"
    )
...
class Subscription(TimestampMixin):
    ...
    max_interval_days = models.IntegerField(
        default=3, help_text="Maximum review interval in days"
    )
    start_date = models.DateTimeField(
        auto_now_add=True, help_text="Subscription start date"
    )
    end_date = models.DateTimeField(
        null=True, blank=True, help_text="Subscription end date (null for unlimited)"
    )
    ...
    auto_renewal = models.BooleanField(
        default=True, help_text="Whether to auto-renew subscription"
    )
    next_billing_date = models.DateTimeField(
        null=True, blank=True, help_text="Next automatic billing date"
    )
...
class PaymentHistory(models.Model):
    ...
    payment_method_used = models.CharField(
        max_length=100, blank=True, help_text="Payment method used for this transaction"
    )
    ...
    gateway_order_id = models.CharField(
        max_length=200, blank=True, null=True, help_text="Order ID from gateway"
    )
    ...
    def __str__(self):
        return (
            f"{self.user.email} - {self.get_payment_type_display()} - {self.created_at}"
        )
    ...
    def tier_display(self):
        ...
        if self.from_tier and self.from_tier != self.to_tier:
            from_display = dict(SubscriptionTier.choices).get(
                self.from_tier, self.from_tier
            )
...
class BillingSchedule(TimestampMixin):
    ...
    subscription = models.ForeignKey(
        Subscription, on_delete=models.CASCADE, related_name="billing_schedules"
    )
    scheduled_date = models.DateTimeField(help_text="Date when billing should occur")
    amount = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Amount to be billed"
    )
    billing_cycle = models.CharField(
        max_length=20,
        choices=BillingCycle.choices,
    ...
    payment_method = models.CharField(
        max_length=100, blank=True, help_text="Payment method to use for billing"
    )
    notes = models.TextField(
        blank=True, help_text="Additional notes about this billing schedule"
    )
    processed_at = models.DateTimeField(
        null=True, blank=True, help_text="When this billing was processed"
    )
    ...
    def is_due(self):
        ...
        return (
            ...
            and self.status == self.ScheduleStatus.PENDING
        )

    def can_be_processed(self):
        """Check if this billing can be processed"""
        return (
            self.status == self.ScheduleStatus.PENDING
    ...
    if created:
        try:
            ...
            Subscription.objects.create(
                user=instance, tier=SubscriptionTier.BASIC, max_interval_days=90
            )
...
class NotificationPreference(TimestampMixin):
    ...
    email_notifications_enabled = models.BooleanField(
        default=True, help_text="이메일 알림 전체 활성화/비활성화"
    )

    # 복습 알림 설정
    daily_reminder_enabled = models.BooleanField(
        default=True, help_text="일일 복습 알림 활성화"
    )
    daily_reminder_time = models.TimeField(
        default="09:00", help_text="일일 복습 알림 시간"
    )

    # 저녁 리마인더 설정
    evening_reminder_enabled = models.BooleanField(
        default=True, help_text="저녁 복습 리마인더 활성화"
    )
    evening_reminder_time = models.TimeField(
        default="20:00", help_text="저녁 복습 리마인더 시간"
    )

    # 주간 요약 설정
    weekly_summary_enabled = models.BooleanField(
        default=True, help_text="주간 요약 이메일 활성화"
    )
    ...
    weekly_summary_time = models.TimeField(
        default="09:00", help_text="주간 요약 발송 시간"
    )

    # 구독 해지 토큰
    unsubscribe_token = models.CharField(
        max_length=64, unique=True, blank=True, help_text="이메일 구독 해지용 토큰"
    )
