...
class EmailTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def validate(self, attrs):
        ...
        if email and password:
            ...
            if (
                getattr(settings, "ENFORCE_EMAIL_VERIFICATION", False)
    ...
    def get_subscription(self, obj):
        ...
        if hasattr(obj, "subscription"):
            return {
                ...
                "start_date": (
                    obj.subscription.start_date.isoformat()
                    if obj.subscription.start_date
                    else None
                ),
                "end_date": (
                    obj.subscription.end_date.isoformat()
                    if obj.subscription.end_date
                    else None
                ),
...
class ProfileSerializer(serializers.ModelSerializer):
    ...
    def validate_weekly_goal(self, value):
        ...
        if value is not None:
            if value < 1:
                raise serializers.ValidationError(
                    "주간 목표는 최소 1회 이상이어야 합니다."
                )
            if value > 1000:
                raise serializers.ValidationError(
                    "주간 목표는 최대 1000회까지 설정 가능합니다."
                )
        return value
...
class UserRegistrationSerializer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if email_field:
            ...
            email_field.validators = [
                v for v in email_field.validators if not isinstance(v, UniqueValidator)
            ]
    ...
    def create(self, validated_data):
        ...
        try:
            # Remove password_confirm from user creation data
...
class SubscriptionUpgradeSerializer(serializers.Serializer):
    ...
    def validate_tier(self, value):
        ...
        if (
            ...
            and user.subscription.tier == SubscriptionTier.PRO
        ):
