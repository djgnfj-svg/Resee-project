...
class ReviewSchedule(TimestampMixin, UserOwnedMixin):
    ...
    content = models.ForeignKey(
        "content.Content", on_delete=models.CASCADE, related_name="review_schedules"
    )
    ...
    initial_review_completed = models.BooleanField(
        default=False, help_text="Whether the initial review has been completed"
    )
...
class ReviewHistory(TimestampMixin, UserOwnedMixin):
    ...
    content = models.ForeignKey(
        "content.Content", on_delete=models.CASCADE, related_name="review_history"
    )
    ...
    ai_score = models.FloatField(
        null=True, blank=True, help_text="AI evaluation score (0-100)"
    )
    ai_feedback = models.TextField(
        null=True, blank=True, help_text="AI generated feedback"
    )
    ...
    user_title = models.CharField(
        max_length=100, blank=True, help_text="User guessed title for subjective mode"
    )
