...
class TodayReviewViewTest(TestCase):
    ...
    def test_get_today_reviews_category_filter(self):
        ...
        content2 = Content.objects.create(
            title="Content 2", content="Body 2", author=self.user, category=category2
        )
        ...
        response = self.client.get(
            f"/api/review/schedules/today/?category_slug={self.category.slug}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class CompleteReviewViewTest(TestCase):
    ...
    def test_complete_review_remembered(self):
        ...
        response = self.client.post(
            f"/api/review/schedules/{self.schedule.id}/completions/",
            {"result": "remembered", "time_spent": 120, "notes": "Good review"},
            format="json",
        )
        ...
        history = ReviewHistory.objects.filter(
            content=self.content, user=self.user
        ).first()
    ...
    def test_complete_review_invalid_time_spent(self):
        ...
        response = self.client.post(
            f"/api/review/schedules/{self.schedule.id}/completions/",
            {"result": "remembered", "time_spent": -10},
        )
    ...
    def test_complete_review_time_spent_too_long(self):
        ...
        response = self.client.post(
            f"/api/review/schedules/{self.schedule.id}/completions/",
            {"result": "remembered", "time_spent": 86401},  # 24 hours + 1
        )
    ...
    def test_complete_review_notes_too_long(self):
        ...
        response = self.client.post(
            f"/api/review/schedules/{self.schedule.id}/completions/",
            {"result": "remembered", "notes": "x" * 5001},  # Exceeds 5000 char limit
        )
...
class ReviewHistoryViewSetTest(TestCase):
    ...
    def setUp(self):
        ...
        ReviewHistory.objects.create(
            content=self.content, user=self.user, result="remembered", time_spent=120
        )
        ReviewHistory.objects.create(
            content=self.content, user=self.user, result="forgot", time_spent=90
        )
