...
class ReviewNotificationService:
    ...
    @staticmethod
    def get_weekly_summary_data(user: User, week_start: datetime, week_end: datetime) -> Dict:
        """
        특정 주에 대한 사용자의 복습 요약 데이터를 생성
        ...
        return {
            ...
            'daily_breakdown': ReviewNotificationService._get_daily_breakdown(week_reviews, week_start, week_end)
        }

    @staticmethod
    def _get_daily_breakdown(reviews, week_start: datetime, week_end: datetime) -> List[Dict]:
        """주간 리뷰의 일별 분석"""
        daily_data = []
    ...
    @staticmethod
    def get_upcoming_reviews(user: User, days_ahead: int = 7) -> List[Dict]:
        """
        향후 N일간의 복습 예정 스케줄을 조회

        Args:
            user: 사용자 객체
            days_ahead: 조회할 일수 (기본 7일)

        Returns:
            List[Dict]: 일별 복습 예정 정보
        """
        start_date = timezone.now().date()
        end_date = start_date + timedelta(days=days_ahead)

        upcoming_schedules = ReviewSchedule.objects.filter(
            user=user,
        ...
        for schedule in upcoming_schedules:
            date_key = schedule.next_review_date.date()
            if date_key not in daily_schedules:
                daily_schedules[date_key] = []
            daily_schedules[date_key].append(schedule)

        # 일별 정보 구성
        daily_data = []
        current_date = start_date
        while current_date <= end_date:
            schedules_today = daily_schedules.get(current_date, [])
            daily_data.append({
                'date': current_date,
                'day_name': current_date.strftime('%A'),
