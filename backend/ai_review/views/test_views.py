...
class AIAnalyticsView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # Calculate study pattern (mock data for now)
            study_pattern = {}
            for i in range(7):
                day_reviews = reviews.filter(review_date__week_day=i+1).count()
                study_pattern[f'day_{i}'] = day_reviews
            
            # Mock insights (in a real implementation, this would use AI)
            insights = []
            if success_rate > 80:
                insights.append({
                    'type': 'positive',
