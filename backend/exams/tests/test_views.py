...
class WeeklyTestListCreateViewTest(TestCase):
    ...
    def test_list_weekly_tests(self):
        ...
        WeeklyTest.objects.create(user=self.user, title="Test 1")
        WeeklyTest.objects.create(user=self.user, title="Test 2")
    ...
    def test_create_weekly_test_with_content_ids(self, mock_simple, mock_ai_available):
        ...
        response = self.client.post(
            "/api/exams/", {"title": "My Test", "content_ids": content_ids}
        )
    ...
    @patch("exams.views.WeeklyTestListCreateView._create_simple_question")
    def test_create_weekly_test_auto_balance(
        self, mock_simple, mock_balance, mock_ai_available
    ):
    ...
    def test_create_weekly_test_ai_unavailable(self, mock_ai_available):
        ...
        response = self.client.post(
            "/api/exams/", {"title": "Test", "content_ids": content_ids}
        )
    ...
    def test_create_weekly_test_limit_exceeded(self):
        ...
        WeeklyTest.objects.create(user=self.user, title="This Week Test")

        content_ids = [c.id for c in self.contents[:7]]

        # Try to create another
        response = self.client.post(
            "/api/exams/", {"title": "Second Test", "content_ids": content_ids}
        )
...
class WeeklyTestDetailViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.test = WeeklyTest.objects.create(
            user=self.user, title="Original Title", status="pending"
        )
    ...
    def test_update_weekly_test(self):
        ...
        response = self.client.patch(
            f"/api/exams/{self.test.id}/", {"title": "Updated Title"}
        )
...
class StartTestViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.content = Content.objects.create(
            title="Test", content="Test body", author=self.user, category=self.category
        )

        self.test = WeeklyTest.objects.create(user=self.user, status="pending")
    ...
    def test_start_test(self):
        ...
        response = self.client.post("/api/exams/start/", {"test_id": self.test.id})
    ...
    def test_start_test_not_exists(self):
        ...
        response = self.client.post("/api/exams/start/", {"test_id": 99999})
...
class SubmitAnswerViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.test = WeeklyTest.objects.create(
            user=self.user, status="in_progress", total_questions=1
        )
    ...
    def test_submit_answer_correct(self):
        ...
        response = self.client.post(
            ...
            {"question_id": self.question.id, "user_answer": "4"},
        )
    ...
    def test_submit_answer_incorrect(self):
        ...
        response = self.client.post(
            "/api/exams/submit-answer/",
            {"question_id": self.question.id, "user_answer": "5"},
        )
    ...
    def test_submit_answer_invalid_question(self):
        ...
        response = self.client.post(
            "/api/exams/submit-answer/", {"question_id": 99999, "user_answer": "A"}
        )
    ...
    def setUp(self):
        ...
        for i in range(2):
            ...
            WeeklyTestAnswer.objects.create(
                question=question, user=self.user, user_answer="A"
            )

    def test_complete_test(self):
        """Test completing a test."""
        response = self.client.post("/api/exams/complete/", {"test_id": self.test.id})
...
class TestResultsViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.answer = WeeklyTestAnswer.objects.create(
            question=self.question, user=self.user, user_answer="A"
        )
