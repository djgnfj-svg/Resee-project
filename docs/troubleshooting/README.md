...
- [08. Gunicorn Worker 최적화](./08-gunicorn-worker-최적화.md)
  - Redis Throttling 멀티 워커 호환성 검증
  - Worker 수 증가 (1→2) 및 성능 2배 향상
  - t3.small 환경 최적화 (2 vCPU, 2GB RAM)
...
8. **[Gunicorn Worker 최적화](./08-gunicorn-worker-최적화.md)** (중급)
   - **문제**: Worker 1개로 제한되어 성능 병목 (동시성 2개)
   - **해결**: Redis Throttling 멀티 워커 호환성 검증 후 Worker 2개로 증가
   - **결과**: 동시성 2배 향상 (2→4), CPU 활용 100%
   - **환경**: t3.small (2 vCPU, 2GB RAM)
...
- **2025-10-20**: 초기 버전 작성 (5개 문서)
  - N+1 쿼리 최적화
  - Database NULL 제약 조건
  - 이메일 토큰 보안
  - Celery 시작 순서
  - Gunicorn Worker 최적화 (추가)
