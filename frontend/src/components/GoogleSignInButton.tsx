...
const GoogleSignInButton: React.FC<GoogleSignInButtonProps> = ({
  ...
  if (!clientId || disabled) {
    const isDisabledForTerms = disabled && clientId;
    return (
      <div className="w-full">
        <button
          type="button"
          disabled
          className={`w-full flex items-center justify-center px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 font-medium cursor-not-allowed opacity-60`}
        >
          ...
          {isDisabledForTerms ? buttonText : `${buttonText} (준비 중)`}
        </button>
        {isDisabledForTerms && (
